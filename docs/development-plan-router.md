...
> **当前 profile 实况（2026-08-18）**：`~/.dsh/profiles/web/cordis.patch.yml` 已配 `router.mode: cost`、primary=`deepseek-v4-flash`、premium/premiumLong=`k3`、`premiumBudget: 0.2`；**`escalateWhen.patterns: [看图, 图像, 截图, 审查, review]` 已配置（2026-08-18 合并进路由器修复实施，用户重启 dsh web 后生效）**——cost 模式的关键词升级现在会触发。配置前（Round 1 时）未配 `escalateWhen`，除显式 `@kimi` 外恒走 primary；已随本轮实机复测验证。
...
| **M5** 实机集成验证 | 装 profile 重启，验证：普通任务走 deepseek、@kimi 走 kimi、图片走 kimi；dock 渲染/命令往返/持久化 | 会话日志 request/header 观察路由 | 🟡 进行中（profile 已配 `mode: cost` + `escalateWhen.patterns`，用户已重启 dsh web；2026-08-18 Round 2 文档子代理即首轮实机复测，见 §2.1 注） |
