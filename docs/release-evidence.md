...
| 1.1.0 评审流认领语义 | ⏸ **已实施 + 实机验收 A1–A7 全绿（2026-09-04）+ A8 缺陷挂起**（试一句缺「评审模型不可用」标注；修复并复测后方满足全绿门禁；已并入 main，未打 tag） | 认领组静态抑制 / 轮末 turn-stopping 异步评审（零阻塞+防环）/ `kimi-tide/review` 投影+事件卡双端 / `/kimi-tide review` 手动命令 / 设置页认领提示+试一句预测 / L7 校验加固（`2d1ebd5`..`9051b1b`，含 A6 实机缺陷修复）；验收逐项记录见 [`superpowers/plans/2026-09-04-review-flow-orchestration.md`](superpowers/plans/2026-09-04-review-flow-orchestration.md) 末节；544/544 绿 + typecheck 0 |
