...
- [ ] **A8** 盲区可见性：配置「组认领 + 评审模型不可用」→ 试一句仍显示 review-flow 并标注「评审模型不可用」 ❌ **缺陷：标注缺失（见验收记录）**
...
- **逐项结果**：
  ...
  - **A8 ❌** reviewer 指向 `ghost-provider/no-such-model`（协作流页确认热加载生效）+ 组认领生效：试一句重算后仍显示「轮末触发评审流 review」但**无「评审模型不可用」标注**（两种输入复测一致）→ **缺陷待修**
- **缺陷与修复**：
  - A6 评审卡注册晚到 → `9051b1b`（uiConversation 缺席改 ctx.inject 延迟驱动 + 5 测试用例），本次实机验证通过
  - **A8 缺陷（试一句缺「评审模型不可用」标注）→ 待修**（fix round 候选），修复前不满足全绿门禁
...
- **技术门禁**：544/544 测试绿 + typecheck 0 错（`9051b1b` 工作树，2026-09-05 00:04）
- **用户裁定**：待定——A8 修复并复测后 tag v1.1.0，或用户裁定豁免发版
