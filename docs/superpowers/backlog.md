...
- **状态**：**部分完成（2026-09-15）**——说明页签 spec 已升 v2 **并实施完毕**（提交 `eece851`：八分区内容、状态感知 live 行、症状→原因表、单一内容源、防腐烂闸、dock 13 处 `data-kt-el` 锚、页签容器 + `hidden` 取代 `:not()` 链、tabpanel/aria-controls/方向键）；**余下两份 spec（用量余额 / 语义闸）的 v2 回改与实施仍待做**
