...
- **状态**：**进行中（2026-09-15）**——① 说明页签：spec v2 + **实施完毕**（`eece851`）；② 用量/余额：spec v2 + **实施完毕**（`36ff9ee` 数据层与自适应槽、`bb24af4` 总览面板、`909577d` Q5 结论与文档）——四源注册表、`QuotaLike` 拓宽、余额解析与三段 baseURL 链、`quotaSources` 三态元数据、dock 余额单槽 + 用量总览面板；③ 语义闸：spec v2 + **主体实施完毕**（`ccf82e1`：hit-confirm.ts 判官闸、`preset.hitConfirm` 不入 schema、`decide` 第 4 参与标注解耦、pre-step 前置短路、三处调用同带判否集），**余设置页开关（spec §8.1）与实机验收 A1–A6**
