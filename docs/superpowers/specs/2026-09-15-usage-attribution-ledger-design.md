...
> 状态：**待评审**（按仓库纪律，实施前先出稿 + 独立评审）
> 动因：用户提出「每次对话都有这个用量统计，可以用这个信息」→ 追问后裁定 **A（按模型/规则/预设的真实用量与成本账）+ C（成本/用量接回路由决策）**，并追加一条更关键的用途：**用「用量归属」当路由是否真的生效的判据**。
...
- **配对**：按 **`turn`** 配对（两侧都有 `turn`）。同一轮内若出现**多个不同** model（重试、转述、判官调用等），取**第一条**非 interrupted 的 `assistant/message` 作为「本轮实际服务方」。

### 4.2 判定与措辞

| 情形 | 判定 | 展示 |
| --- | --- | --- |
| 决策 target == 实际 source | ✅ 一致 | 不产生记录（零噪声） |
| 不等且决策 `via:'rule'/'explicit'` | ❌ **路由未生效** | 决策串追加注记「实际由 `p/m` 作答（与决策不符）」；账本 `mismatch` +1 |
| 不等且决策 `via:'default'` | ⚠️ 打底被改（可能是宿主模型选择覆盖、也可能是用户手动换模型） | 同上，但措辞标「打底被覆盖」 |
| 该轮无决策（router 未挂载 / keep） | 不判 | 只记账 |
| 该轮无 assistant/message（中止） | 不判 | 只记账 |
...
**配置面（拟，全部可选、缺省关闭 ⇒ 存量零突变）**：

```yaml
kimi-tide-router:
  usageLedger:
    enabled: true          # 面板显示本轮/本会话账
    attributeCheck: true   # 归属判据（不符时在决策串里报）
    prices:                # 可选；单位 = 元 / 百万 token
      deepseek-official/deepseek-flash: { in: 1.0, cacheRead: 0.1, out: 4.0 }
      zai-coding-cn/glm-5.3: { in: 4, cacheRead: 0.4, out: 16 }
```

`prices` 缺省 ⇒ 只报 token、不报钱（**绝不内置价格**）。

**配置面的落法有先例约束（必须先读再写）**：仓库自述过 schemastery 的行为——**对象/字典/数组型字段缺失即注入 `{}`/`[]`**（`settings-schema.ts:63-71`，v1.3.0 的语义闸 spec 评审 S1 正是被这一条打回：把 `hitConfirm` 写成 `Schema.object` 会破坏 `settings-schema.test.ts:7-9` 的「默认往返逐字节相等」）。`prices` 是**字典型**、且配置是**嵌套在 preset 之外**的，故本稿的落法照 `hitConfirm` 的既有先例：**不入 schema、靠未知键透传保活 + `validateRouterConfig` 校验**，并补一条「默认往返仍逐字节相等」的测试钉住它。实施前必须先实读 `coerceRouterConfigV5`/`RouterSettingsStore` 的往返路径确认透传成立（**未核实不得开工**）。

## 6. 未决（实施前需裁定）

1. **要不要钱**：只报 token（零维护风险）／加可选单价表（用户自填）。
2. **账的时间窗**：只本会话／本会话 + 跨会话累积（跨会话需要落盘，与「不写会话日志」不冲突，但要选落点：sidecar？settings 命名空间？）。
3. **每条规则的账**：把 `turn → 当时生效的 ruleId` 也记下来，才能答「哪条规则在烧钱」。要，还是先只按模型？
4. **C 组怎么落**：只做「账摆在旁边 + 允许按单价排序」／还是要让价格真正参与路由选择（后者需要先有价格，否则是空转）。
5. **实施顺序**：本稿一次做完 A+C，还是先做「归属判据」（最小、立刻有用、直接服务今天的验收），账本与价格随后。
