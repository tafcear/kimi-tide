...
> 状态：设计定稿待评审（2026-08-22 脑暴 + k3 独立评审 + 用户三裁定 + 架构批准）→ **已实施并发布（2026-08-23，tag `v0.6.0`，[Release](https://github.com/tafcear/kimi-tide/releases/tag/v0.6.0)）：P1 全量落地，实机验收 10 项全过（含 T4 门，用户裁定通过）；验收修复 `e2d3c68`（rc.2 宿主 model-selection 覆盖路由）；面板图像上下文行客户端渲染降级 0.6.x**
...
- **行为保持**：迁移后路由行为与 0.5.0 逐字节一致；用户经设置页把 image 规则改挂 `flow:transcribe` 才启用新姿态。行为保持的「逐字节」范围=**无流绑定且默认打底路径**；带图轮之后的关键词命中轮按 §5.1 新语义走关键词规则（0.5.0 的图像规则 hijack 是布尔锁存副作用，不复活——锁存/latch fallback 只负责防崩防盲，不负责劫持规则链，新旧落点均多模态、不崩不盲）。
...
> **验收结果（2026-08-23 回写）**：1-8、10 全过 ✅；9 属 P2 不适用 ⏭️。过程实录（会话日志锚点）：①迁移 `.pre-v5` 留档+行为保持 ②设置页流注册表目检通过 ③eager 转述机制级通过（转述文字进上下文+文本模型作答含图；首测瞬时失败重启后自愈）④切回+投影帧 transcribed=1 持久化 ⑤latch「带图锁存改道」→vision-exp（source 实锤）⑥blind decision null+source v4-pro 盲答 ⑦看原图重挂→vision-exp 原生 ⑧**T4 门用户裁定通过**（报错截图转述→仅凭转述诊断可定位可处置）⑩官方账单 vision-exp ¥1.54 实锤。
> **验收中修复 1 阻断缺陷**（`e2d3c68`）：rc.2 宿主 `installModelSelection` 覆盖监听器把路由结果覆盖回会话选定模型（面板决策对、实际请求恒 session 模型）——四监听器 `{prepend:true}` 恒外层修复；**验收中降级 0.6.x 2 项**（用户裁定）：①面板图像上下文行/流事件客户端渲染缺失（宿主投影 v6 数据已推送，TideDock/ReasonPanel 零消费——Task 10 只交付投影 schema，客户端行漏交付）②转述首测瞬时失败无留证。

1. v4→v5 迁移留档（`.pre-v5`）+ 行为保持（迁移前后同输入同路由）。 ✅
2. 设置页流注册表可见可编；image 规则改挂 transcribe 流并保存生效。 ✅
3. eager：带图消息 → 转述事件上面板 → 文本模型作答含图信息（非盲答）。 ✅（机制级；面板显示降级）
4. 切回：转述完成后下一轮文本消息 → 路由回文本模型且面板图像上下文行 transcribed=1。 ✅（投影帧实锤；面板显示降级）
5. `imageFallback=latch`：native 图后文本轮仍走视觉（按图锁存实锤）。 ✅
6. `blind`：文本轮占位盲答 + 警示态显示。 ✅（机制级；警示态随面板渲染降级）
7. 「看原图」：该轮重挂原图走视觉（会话日志 request/header 实锤）。 ✅
8. T4 门（§10）。 ✅（用户裁定：诊断可用）
9. review 命令：评审意见可见进会话、rounds 上限生效。（P2 落地后） ⏭️ N/A
