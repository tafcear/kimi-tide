...
> 状态：**v1（2026-09-15，随 v1.3.0 并入）**——用户裁定「Q6 并入这一版，完成后再发」
...
- 语义闸短路：`@README.md` + `hitConfirm.enabled` ⇒ 闸**运行**（不再是显式轮短路）。
...
| R5 | 本项为**行为修正**，未做独立模型评审（v1.3.0 同批的另三项各有独立评审档案） | 用户裁定「并入这一版，完成后再发」；如需补评审，可在 tag 前加一轮（控制器先出 spec 即为此留出评审面） |
