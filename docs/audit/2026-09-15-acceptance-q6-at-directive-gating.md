...
- 评审者的 M3/M4/M5 三个变异未实跑（其会话被截止）；**本会话已把三条全部补跑**：
  - **M3（首已知匹配 → 只看词法首个匹配）⇒ `rules.test.ts:67-69` 一条红**（精确命中，证明「首个已知匹配」语义真被钉住）；
  - **M4（knownProviders 滤 available）⇒ 新补用例一条红**（见上表 M1 行）；
  - **M5（`router.ts:881` 漏传第 4 参）⇒ 新补 4b 一条红**（见上表 M2 行）。
  三次变异均**精确命中单一用例**、无连带噪声，说明这几处的判别力是干净的。
- A9 实机验收、R6（preview/decide 两次拉取口径分叉，需宿主活体）、R1（枚举失败窗口期行为）离线不可构造，维持 spec 备案。
- 本会话发现的补充边界：`known == null` 时门控退化为纯词法判定（见 A4），spec 未写成风险。
- **L3 的边界（已知 provider 同名碰撞）未修**：`@kimi.md`、`@kimi-tide/kimi-tide` 这类文本仍会被判为指令（词法固有限度，Q6 前后行为相同、**非回归**）。当前**无用户可感知的坏结果**：`@kimi.md` 会走 `keep`（已知别名但无可用候选）或精确寻址路径，且原因串会写出实际模型——所以只登记不修，避免为此收紧词法而误伤 `@kimi帮我看` 这类无空格真指令。
