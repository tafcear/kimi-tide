...
> **当前状态**：v1.2.1（2026-09-11，[Releases](https://github.com/tafcear/kimi-tide/releases)），590/590 测试绿。本版修复设置页模型下拉：目录通道迁移到宿主官方模型目录接口（DSH 0.1.5-rc.1 起不再丢掉 DeepSeek 整组）+ 下拉按提供方分组、显示官方友好名。1.2.0 起面板数据退出会话日志。版本历史见仓库根 [CHANGELOG](../../CHANGELOG.md)；项目介绍与快速开始见[根 README](../../README.md)。匹配语义（词边界/特异度排序/最少命中词数）、effort 推理档位与路由配置全字段，见 [docs/router.md](docs/router.md)。
