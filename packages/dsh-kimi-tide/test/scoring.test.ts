...
describe('scoring', () => {
  ...
  it('code-heavy weights pick the stronger code model despite cost', () => {
    ...
    const b = meta('kimi-coding', 'k3', { costTier: 'mid' })
  ...
  it('vision=0 candidates are excluded for image steps', () => {
    const a = meta('kimi-coding', 'k3', { modalities: ['text', 'image'] })
    ...
    expect(sel!.target.provider).toBe('kimi-coding')  // deepseek-v4-flash（vision=0）被排除；k3 多模态胜出
  })
