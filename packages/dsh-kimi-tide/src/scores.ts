...
export const SCORES_VERSION = 3

// code（SWE-bench，agentic coding 事实标准）：
//   kimi-coding/k3                93.4% → 4.7  一级  [https://modelfit.io/blog/can-you-run-kimi-k3-locally/]
//                                             官方 vs 独立口径差异见 [https://apidog.com/blog/kimi-k3-benchmarks/]
//   kimi-coding/kimi-for-coding   → 4.5         推断 编码专用模型（Kimi Code K2.x，SWE-bench 逼近 Opus）
...
//   kimi-coding/k3                → 4.5         推断 frontier 同级（对标 Claude Opus 4.8 / GPT 5.5）
//                                             [https://www.digit.in/features/general/what-is-kimi-k3-...]，GPQA 精确值待核实
//                                             [https://matharena.ai/models/moonshot_k3]
//   kimi-coding/kimi-for-coding   → 3.5         推断 编码专用、通用推理偏弱；待核实
...
const BASELINE: Record<string, Partial<Record<Dim, number>>> = {
  'kimi-coding/k3': { code: 4.7, reasoning: 4.5 },
  'kimi-coding/kimi-for-coding': { code: 4.5, reasoning: 3.5 },
  'deepseek-official/deepseek-v4-flash': { code: 3.0, reasoning: 3.0 },
  'deepseek-official/deepseek-v4-pro': { code: 4.0, reasoning: 4.5 },
}

export function scoreFor(cfg: RouterConfigV3, target: RouteTarget): Record<Dim, number> {
